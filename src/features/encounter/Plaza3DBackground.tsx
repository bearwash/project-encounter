...
// 起動時のロード段付き（ポップイン）を抑えるため、全モデルを先読みする。
Object.values(MODEL_FILES).forEach((file) => {
  if (file === MODEL_FILES.meshyPlaygroundSlide) return;
  useGLTF.preload(`${MODEL_ROOT}${file}`);
});
type ModelPlacement = {
  key: string;
  model: ModelKey;
...
const SHADOW_CASTER_MODELS = new Set<ModelKey>([
  'meshyLowPathTree',
  'roundTopiary',
  'meshyParkEntranceGate',
  'meshyParkEntranceSign',
  'streetLamp',
  'parkBench',
  'fountain',
  'meshyParkGazebo',
  'meshyPlaygroundSlide',
]);
